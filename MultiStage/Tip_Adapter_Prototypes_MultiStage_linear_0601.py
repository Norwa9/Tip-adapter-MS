...
def main():
    ...
    if load_adapter:
        ...
        for train_idx in range(args.train_epoch):
            ...
            acc1,acc2,acc3,acc4,acc5,acc10 = accuracy(logits, test_labels, topk=(1,2,3,4,5,10))
            top1 += acc1
            top2 += acc2
            top3 += acc3
            top4 += acc4
    ...
    if not load_testing_prototypes:
        ...
        with torch.no_grad():
            for i, (images, target) in enumerate(tqdm(loader)):
                ...
                # topK_protos:[batch,topK,1024]
                # topK_zeroshot_weights:[batch,topK,1024]
    ...
    for train_idx in range(args.refine_epoch):
        ...
        for i, (images, target) in enumerate(tqdm(train_loader_shuffle)):
            ...
            logits, _ = adapter(image_features, target)
            topK_protos, topK_zeroshot_weights = get_topK_ProtosAndZeroshotWeight(logits,adapter.proto,adapter.zero_shots_weight,args.topK)
            new_logits = transformer(topK_protos) # [batch, 1000]
        ...
        with torch.no_grad():
            for i, (images, target) in enumerate(tqdm(loader)):
                ...
                target = target.cuda()
                new_logits = transformer(test_prototypes[i*batch:(i+1)*batch])
                acc1,acc5,acc10 = accuracy(new_logits, target, topk=(1,5,10)) # new_target: [batch, topK+1]
                top1 += acc1
                top5 += acc5
                top10 += acc10
                n += batch
...
# python /data/luowei/missing_modality/Tip-Adapter-Multi-Stage/MultiStage/Tip_Adapter_Prototypes_MultiStage_linear_0601.py
