...
'''
class CLIP_Tip_Adapter(nn.Module):
...
def main():
    ...
    adapter = CLIP_Tip_Adapter(clip_model=model, train_features_path=train_features_path, cls_num=len(imagenet_classes), shots=k_shot).cuda()
    if load_adapter:
    ...
    # ------------------------------------------ Stage2 : Finetune CLIP using topK class masks ------------------------------------------

    # adapter_extractor
    # 用于提取batch样本的mask
    adapter_extractor = CLIP_Tip_Adapter(clip_model=model,train_features_path=train_features_path, cls_num=len(imagenet_classes), shots=k_shot).cuda()
...
# python /data/luowei/missing_modality/Tip-Adapter-Multi-Stage/MultiStage/ClipAdapter_topMask_0526.py
